...
def withoutRT():
    ...
    outData = list()
    outString = ""
    outputFileName = ("{}/data/countNoRT".format(dir_path))
    for day in days:
        YEAR, MONTH, DAY, HOUR = day.split('-')
        inputDirLocation = ("{}/data/stance/{}{}{}{}_keywords.json".format(dir_path, YEAR, MONTH, DAY, HOUR))
        data = []
        ...
        file.close();
        for item in data:
            if item["text"][0] not in ["R"] and item["text"][1] not in ["T"]:
                total += 1
    print(total)
    outData.append(['Total', str(total)])
    for l in outData:
...
def uniqueUser():
    userDict = dict()
    outString = ""
    outputFileName = ("{}/data/tweetCountByUser.csv".format(dir_path))
    for day in days:
        YEAR, MONTH, DAY, HOUR = day.split('-')
        inputDirLocation = ("{}/data/stance/{}{}{}{}_keywords.json".format(dir_path, YEAR, MONTH, DAY, HOUR))
        data = []
        ...
        file.close();
        for item in data:
            if item["user_id"] not in userDict:
                userDict[item["user_id"]] = 1
            else:
                userDict[item["user_id"]] += 1
    print(len(userDict))
    outString += "totalUserCount"
    outString += ","
    outString += str(len(userDict))
    outString += "\n"
